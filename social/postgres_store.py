...
class SocialPostgresStore:
        ...
        *,
        ...
        WITH candidates AS (
          ...
          LEFT JOIN public.social_processing_failures AS failure
           ...
           AND failure.scope_key = sea.source_key
           AND failure.resolved_at IS NULL
          WHERE sea.is_active = TRUE
            AND se.is_active = TRUE
            AND (%s::text[] IS NULL OR sea.platform = ANY(%s))
            AND COALESCE(NULLIF(BTRIM(sea.account_external_id), ''), NULLIF(BTRIM(sea.account_handle), ''), NULLIF(BTRIM(sea.domain), '')) IS NOT NULL
            AND (sea.next_collect_after IS NULL OR sea.next_collect_after <= NOW())
            AND (
          ...
          ORDER BY COALESCE(sea.next_collect_after, sea.last_collected_at, to_timestamp(0)) ASC, sea.updated_at ASC
          LIMIT %s
        ...
        with self._connection() as conn:
            with conn.transaction():
                with conn.cursor() as cur:
                    cur.execute(query, (platforms or None, platforms or None, lease_seconds, limit, worker_id))
    ...
    def claim_collect_accounts(
        ...
        *,
        ...
        return self.claim_collect_sources(
            worker_id=worker_id,
            platforms=platforms,
            limit=limit,
