...
from social.contracts import (
    ...
    SUPPORTED_SOCIAL_PLATFORMS as CONTRACT_SUPPORTED_SOCIAL_PLATFORMS,
    build_activity_uid,
    build_source_key,
    clean_optional,
    ...
    normalize_platform as contract_normalize_platform,
    normalize_provider_key,
    normalize_target_type,
    source_identifier_fields,
)

SUPPORTED_SOCIAL_PLATFORMS = CONTRACT_SUPPORTED_SOCIAL_PLATFORMS
...
def _normalize_platform(value: Any) -> str:
    return contract_normalize_platform(value)
...
def _normalize_cadence_minutes(value: Any) -> int | None:
    if value in (None, "", 0):
        return None
    try:
        minutes = int(value)
...
def _engagement_total(activity: dict[str, Any]) -> int:
    ...
    if not isinstance(metrics, dict):
        return 0
    total = 0
    for key in ("likes", "comments", "shares", "views", "plays", "impressions", "reactions"):
        try:
...
def _scope_key_for_source(source: dict[str, Any]) -> str:
    scope_key = _trimmed(source.get("source_key"))
    if scope_key:
        return scope_key
    identifier = identifier_from_source(source)
    return build_source_key(
        provider_key=source.get("provider_key") or DEFAULT_PROVIDER_KEY,
        platform=source.get("platform"),
        target_type=source.get("target_type") or DEFAULT_TARGET_TYPE_BY_PLATFORM.get(_normalize_platform(source.get("platform"))),
...
class SocialStore:
    ...
    def _count_rows(
        ...
        *,
        ...
        query = self.client.table(table).select("id", count="exact").limit(1)  # type: ignore[arg-type]
        for op, column, value in filters or ():
            if op == "eq":
                query = query.eq(column, value)
            elif op == "in":
    ...
    def get_runtime_snapshot(self, default: dict[str, Any] | None = None) -> dict[str, Any]:
        return self.get_runtime_setting("runtime_snapshot", default or {})

    def save_runtime_snapshot(self, payload: dict[str, Any]) -> dict[str, Any]:
        return self.save_runtime_setting("runtime_snapshot", payload)

    def get_runtime_control_command(self, default: dict[str, Any] | None = None) -> dict[str, Any]:
        return self.get_runtime_setting("control_command", default or {})

    def save_runtime_control_command(self, payload: dict[str, Any]) -> dict[str, Any]:
        return self.save_runtime_setting("control_command", payload)
    ...
    def sync_entities_from_companies(self, company_ids: list[str] | None = None) -> dict[str, Any]:
        ...
        for company in companies:
            ...
            if _clean_optional(company.get("facebook_page_id")):
                identifier = _clean_optional(company.get("facebook_page_id"))
                account_payloads.append(
                    {
                        ...
                        "platform": "facebook",
                        "target_type": "page_id",
                        **source_identifier_fields(target_type="page_id", identifier=identifier),
                        ...
                        "provider_metadata": {},
                        "is_active": bool(company.get("is_active", True)),
                        "next_collect_after": now,
                    }
                )
            if _clean_optional(company.get("instagram_username")):
                identifier = _clean_optional(company.get("instagram_username"))
                account_payloads.append(
                    {
                        "entity_id": entity_id,
                        "provider_key": DEFAULT_PROVIDER_KEY,
                        "platform": "instagram",
                        "target_type": "handle",
                        **source_identifier_fields(target_type="handle", identifier=identifier),
                        "source_key": build_source_key(
                            provider_key=DEFAULT_PROVIDER_KEY,
                            platform="instagram",
                            target_type="handle",
            ...
            if _clean_optional(company.get("google_ads_domain")):
                identifier = _clean_optional(company.get("google_ads_domain"))
                account_payloads.append(
                    {
                        "entity_id": entity_id,
                        "provider_key": DEFAULT_PROVIDER_KEY,
                        "platform": "google",
                        "target_type": "domain",
                        **source_identifier_fields(target_type="domain", identifier=identifier),
                        "source_key": build_source_key(
                            provider_key=DEFAULT_PROVIDER_KEY,
                            platform="google",
                            target_type="domain",
    ...
    def _normalize_source_payload(self, entity_id: str, source: dict[str, Any]) -> dict[str, Any]:
        platform = _normalize_platform(source.get("platform"))
        provider_key = normalize_provider_key(source.get("provider_key") or DEFAULT_PROVIDER_KEY)
        target_type = normalize_target_type(
            source.get("target_type") or DEFAULT_TARGET_TYPE_BY_PLATFORM.get(platform),
            platform=platform,
        )
        explicit_identifier = _clean_optional(source.get("identifier"))
        identifier = explicit_identifier or identifier_from_source(
            {
                "platform": platform,
                "target_type": target_type,
                "account_handle": source.get("account_handle"),
                "account_external_id": source.get("account_external_id"),
                "domain": source.get("domain"),
            }
        )
        if not identifier:
            raise ValueError("Social source identifier is required")
        content_types = normalize_content_types(
            source.get("content_types") or DEFAULT_CONTENT_TYPES_BY_PLATFORM.get(platform),
            platform=platform,
        )
        cadence_minutes = _normalize_cadence_minutes(source.get("cadence_minutes"))
        source_key = build_source_key(
        ...
        payload = {
            ...
            **source_identifier_fields(target_type=target_type, identifier=identifier),
            "content_types": content_types,
            "import_source": _clean_optional(source.get("import_source")) or "manual",
            "metadata": _serialize_metadata(source.get("metadata")),
            "provider_metadata": _serialize_metadata(source.get("provider_metadata")),
            "is_active": bool(source.get("is_active", True)),
            "cadence_minutes": cadence_minutes,
            "next_collect_after": source.get("next_collect_after") or _utc_now_iso(),
        }
        return payload

    def _load_sources_for_entities(self, entity_ids: list[str]) -> dict[str, list[dict[str, Any]]]:
        if not entity_ids:
            return {}
    ...
    def _enrich_entities(self, entities: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        sources_by_entity = self._load_sources_for_entities(entity_ids)
        for entity in entities:
            sources = sources_by_entity.get(str(entity.get("id")), [])
            entity["sources"] = sources
            entity["accounts"] = sources
            entity["platform_accounts"] = {
                platform: next((row for row in sources if row.get("platform") == platform and row.get("is_active", True)), None)
    ...
    def update_entity(
        ...
        *,
        ...
        if sources:
            self.upsert_sources(entity_id, sources)
        elif accounts:
    ...
    def list_sources(
        ...
        *,
        ...
        if not include_inactive:
            filters.append(("eq", "is_active", True))
        sources = self._select_rows(
            "social_entity_accounts",
            filters=filters,
            order_by="platform",
        )
        if not sources:
            return []
        entity_ids = list({str(row.get("entity_id")) for row in sources if row.get("entity_id")})
        entities = {
            row["id"]: row
        ...
        return [
            {
                **dict(source),
                "entity": entities.get(source.get("entity_id")),
            }
            for source in sources
        ]

    def get_source(self, source_id: str) -> dict[str, Any] | None:
        row = self._single_row("social_entity_accounts", filters=(("eq", "id", source_id),))
        if not row:
            return None
    ...
    def get_source_by_scope_key(self, scope_key: str) -> dict[str, Any] | None:
        row = self._single_row(
            "social_entity_accounts",
            filters=(("eq", "source_key", _trimmed(scope_key)),),
        )
        if not row:
            return None
        entity = self._single_row("social_entities", filters=(("eq", "id", row.get("entity_id")),))
    ...
    def upsert_sources(self, entity_id: str, sources: list[dict[str, Any]]) -> list[dict[str, Any]]:
        if not sources:
            return []
        saved_ids: list[str] = []
        for source in sources:
            payload = self._normalize_source_payload(entity_id, source)
            existing = self._single_row(
                "social_entity_accounts",
                filters=(("eq", "source_key", payload["source_key"]),),
            )
            if existing is None:
                existing = self._single_row(
                    "social_entity_accounts",
                    filters=(("eq", "entity_id", entity_id), ("eq", "platform", payload["platform"])),
                )
            if existing:
                self.client.table("social_entity_accounts").update(payload).eq("id", existing["id"]).execute()
                saved_ids.append(str(existing["id"]))
            else:
                response = self.client.table("social_entity_accounts").insert(payload).execute()
                inserted = dict((response.data or [{}])[0])
                if inserted.get("id"):
                    saved_ids.append(str(inserted["id"]))
        sources_by_id = {
            source["id"]: source
            for source in self._select_rows("social_entity_accounts", filters=(("in", "id", saved_ids),))
        }
        return [sources_by_id[source_id] for source_id in saved_ids if source_id in sources_by_id]
    ...
    def upsert_accounts(self, entity_id: str, accounts: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for account in accounts:
            ...
            sources.append(
                {
                    "provider_key": account.get("provider_key") or DEFAULT_PROVIDER_KEY,
                    "platform": platform,
                    "target_type": account.get("target_type") or DEFAULT_TARGET_TYPE_BY_PLATFORM.get(platform),
                    "account_handle": account.get("account_handle"),
                    "account_external_id": account.get("account_external_id"),
                    "domain": account.get("domain"),
                    "content_types": account.get("content_types") or DEFAULT_CONTENT_TYPES_BY_PLATFORM.get(platform),
                    "is_active": bool(account.get("is_active", True)),
                    "cadence_minutes": account.get("cadence_minutes"),
                    "provider_metadata": account.get("provider_metadata") or {},
                    "metadata": account.get("metadata") or {},
                    "import_source": account.get("import_source") or "manual",
                }
            )
        self.upsert_sources(entity_id, sources)
        entity = self.get_entity(entity_id)
    ...
    def get_account(self, account_id: str) -> dict[str, Any] | None:
        return self.get_source(account_id)

    def get_account_by_scope_key(self, scope_key: str) -> dict[str, Any] | None:
        return self.get_source_by_scope_key(scope_key)
    ...
    def update_source(
        ...
        *,
        ...
        if content_types is not None:
            payload["content_types"] = normalize_content_types(content_types, platform=existing.get("platform"))
        if cadence_minutes is not None:
            payload["cadence_minutes"] = _normalize_cadence_minutes(cadence_minutes)
        if provider_metadata is not None:
            payload["provider_metadata"] = _serialize_metadata(provider_metadata)
        if metadata is not None:
            payload["metadata"] = _serialize_metadata(metadata)
        if payload:
            self.client.table("social_entity_accounts").update(payload).eq("id", source_id).execute()
    ...
    def mark_source_collect_success(self, source_id: str, *, next_collect_after: str | None) -> None:
        self.client.table("social_entity_accounts").update(
            {
                ...
                "next_collect_after": next_collect_after,
        ...
        self.clear_failure(
            stage="ingest",
            scope_key=_scope_key_for_source(source),
        )

    def mark_account_collect_success(self, account_id: str) -> None:
        account = self.get_source(account_id)
        if not account:
            return
        cadence_minutes = _normalize_cadence_minutes(account.get("cadence_minutes")) or 360
        next_collect_after = (datetime.now(timezone.utc) + timedelta(minutes=cadence_minutes)).isoformat()
        self.mark_source_collect_success(account_id, next_collect_after=next_collect_after)

    def mark_source_collect_failure(
        self,
        source_id: str,
        *,
        health_status: str,
    ...
    def mark_account_collect_failure(
        ...
        *,
        ...
        self.mark_source_collect_failure(
            account_id,
    ...
    def list_active_accounts(self, platforms: list[str] | None = None) -> list[dict[str, Any]]:
        return self.list_due_sources(platforms=platforms, due_only=False)
    ...
    def list_due_sources(
        ...
        *,
        ...
        normalized_provider_keys = [normalize_provider_key(item) for item in (provider_keys or [])]
        if normalized_platforms:
            filters.append(("in", "platform", normalized_platforms))
        if normalized_provider_keys:
            filters.append(("in", "provider_key", normalized_provider_keys))
        if due_only:
            filters.append(("lte", "next_collect_after", _utc_now_iso()))
        sources = self._select_rows(
            "social_entity_accounts",
            filters=filters,
            order_by="next_collect_after" if due_only else "platform",
            limit=limit,
        )
        if not sources:
            return []
        entity_ids = [str(row.get("entity_id")) for row in sources if row.get("entity_id")]
        entities = {
            row["id"]: row
        ...
        for source in sources:
            ...
            if not entity:
                continue
            if not identifier_from_source(source):
                continue
            enriched.append(
                {
                    **dict(source),
    ...
    def upsert_activities(self, activities: list[dict[str, Any]]) -> list[dict[str, Any]]:
        if not activities:
            return []
        candidate_uids = [str(item["activity_uid"]) for item in activities if item.get("activity_uid")]
        ...
        seen_existing_ids = {row["id"] for row in existing_rows if row.get("id")}
        for activity in activities:
            source_key = _clean_optional(activity.get("source_key"))
            provider_item_id = _clean_optional(activity.get("provider_item_id"))
            if not source_key or not provider_item_id:
                continue
            legacy = self._single_row(
                "social_activities",
                columns=(
                    "id,activity_uid,text_content,ingest_status,analysis_status,graph_status,"
                    "first_seen_at,analysis_version,graph_projection_version,provider_key,"
                ...
                filters=(
                    ...
                    ("eq", "platform", _normalize_platform(activity.get("platform"))),
                    ("eq", "source_key", source_key),
                    ("eq", "provider_item_id", provider_item_id),
                    ("eq", "source_kind", _trimmed(activity.get("source_kind")) or "post"),
                ),
            )
            if legacy and legacy.get("id") not in seen_existing_ids:
                existing_rows.append(legacy)
                seen_existing_ids.add(legacy["id"])
        now = _utc_now_iso()
        existing_by_uid = {row["activity_uid"]: row for row in existing_rows if row.get("activity_uid")}
        existing_by_identity = {
            (
                _trimmed(row.get("provider_key") or DEFAULT_PROVIDER_KEY),
                _trimmed(row.get("platform")),
                _trimmed(row.get("source_key")),
                _trimmed(row.get("provider_item_id")),
                _trimmed(row.get("source_kind")),
            ): row
        ...
        for activity in activities:
            normalized_provider_key = normalize_provider_key(activity.get("provider_key") or DEFAULT_PROVIDER_KEY)
            normalized_platform = _normalize_platform(activity.get("platform"))
            source_kind = _trimmed(activity.get("source_kind")) or "post"
            source_key = _clean_optional(activity.get("source_key"))
            provider_item_id = _clean_optional(activity.get("provider_item_id"))
            candidate_uid = str(activity["activity_uid"])
            existing = existing_by_uid.get(candidate_uid)
            if existing is None and source_key and provider_item_id:
            ...
            uid = str(existing.get("activity_uid") if existing else candidate_uid)
            ...
            payloads.append(
                {
                    ...
                    "provider_context": activity.get("provider_context") or {},
    ...
    def get_overview(self) -> dict[str, Any]:
        entities = self.list_entities()
        failures = self.list_failures(limit=10)
        runs = self.list_recent_runs(limit=12)
        active_entities = sum(1 for entity in entities if entity.get("is_active"))
        activity_count = self._count_rows("social_activities")
        platform_counts = {
            platform: self._count_rows("social_activities", filters=(("eq", "platform", platform),))
            for platform in SUPPORTED_SOCIAL_PLATFORMS
        }
        analysis_status_counts = {
            status: self._count_rows("social_activities", filters=(("eq", "analysis_status", status),))
            for status in ("not_needed", "pending", "analyzed", "failed", "dead_letter")
        }
        account_health_counts = Counter()
        for entity in entities:
            for account in entity.get("sources") or entity.get("accounts") or []:
                account_health_counts[_normalize_health_status(account.get("health_status"))] += 1
        stale_entities = []
        now = datetime.now(timezone.utc)
        for entity in entities:
            active_sources = [source for source in (entity.get("sources") or []) if source.get("is_active", True)]
            last_seen = max(
                (str(source.get("last_collected_at")) for source in active_sources if source.get("last_collected_at")),
                default=None,
            )
            if not active_sources or not last_seen:
        ...
        return {
            ...
            "queue_depth": {
                "analysis": self._count_rows(
                    "social_activities",
                    filters=(("in", "analysis_status", ["pending", "failed"]),),
                ),
                "graph": self._count_rows(
                    "social_activities",
                    filters=(("in", "graph_status", ["pending", "failed"]),),
                ),
            },
            "dead_letter_failures": self._count_rows(
                "social_processing_failures",
                filters=(("eq", "is_dead_letter", True), ("is", "resolved_at", "null")),
            ),
