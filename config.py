...
DASH_DEFAULT_ARTIFACT_SEEDER_ENABLED = _env_bool("DASH_DEFAULT_ARTIFACT_SEEDER_ENABLED", True)
DASH_DEFAULT_ARTIFACT_SEED_ON_STARTUP = _env_bool("DASH_DEFAULT_ARTIFACT_SEED_ON_STARTUP", True)
DASH_DEFAULT_ARTIFACT_REFRESH_MINUTES = max(
    5,
    int(os.getenv("DASH_DEFAULT_ARTIFACT_REFRESH_MINUTES", "30")),
)
DASH_DEFAULT_ARTIFACT_SEED_TIMEOUT_SECONDS = max(
    30.0,
    float(os.getenv("DASH_DEFAULT_ARTIFACT_SEED_TIMEOUT_SECONDS", "90.0")),
)
DASH_V2_FACTS_ENABLED = _env_bool("DASH_V2_FACTS_ENABLED", False)
DASH_V2_COMPARE_ENABLED = _env_bool("DASH_V2_COMPARE_ENABLED", False)
DASH_V2_API_ENABLED = _env_bool("DASH_V2_API_ENABLED", False)
DASH_V2_FRONTEND_READ_ENABLED = _env_bool("DASH_V2_FRONTEND_READ_ENABLED", False)
DASH_V2_DETAILS_ENABLED = _env_bool("DASH_V2_DETAILS_ENABLED", False)
DASH_V2_JOB_OWNER = str(os.getenv("DASH_V2_JOB_OWNER", "worker")).strip().lower() or "worker"
DASH_DEFAULT_RANGE_DAYS = max(1, int(os.getenv("DASH_DEFAULT_RANGE_DAYS", "7")))
DASH_V2_FACT_LOOKBACK_DAYS = max(30, int(os.getenv("DASH_V2_FACT_LOOKBACK_DAYS", "400")))
DASH_V2_FACT_MATERIALIZE_INTERVAL_MINUTES = max(
    5,
    int(os.getenv("DASH_V2_FACT_MATERIALIZE_INTERVAL_MINUTES", "30")),
)
DASH_V2_SLICE_POLL_SECONDS = max(
    1,
    int(os.getenv("DASH_V2_SLICE_POLL_SECONDS", "5")),
)
DASH_V2_SLICE_LEASE_SECONDS = max(
    30,
    int(os.getenv("DASH_V2_SLICE_LEASE_SECONDS", "300")),
)
DASH_V2_RECONCILE_LOOKBACK_DAYS = max(
    1,
    int(os.getenv("DASH_V2_RECONCILE_LOOKBACK_DAYS", "30")),
)
DASH_V2_COMPARE_SAMPLE_DAYS = _env_csv(
