...
class AnalyticsAuthTests(unittest.TestCase):
    ...
    def _freshness_resolution(self) -> dict:
        return {
            "snapshot": {"health": {"status": "ok"}, "generated_at": "2026-03-22T00:00:00Z"},
            "source": "memory",
    ...
    def test_dashboard_allows_unauthenticated_access_when_auth_disabled(self) -> None:
        with patch.object(server.config, "ANALYTICS_API_REQUIRE_AUTH", False), \
             ...
             patch.object(server, "_cached_freshness_resolution", return_value=self._freshness_resolution()), \
             patch.object(server, "_trusted_end_date_from_freshness", return_value=date(2026, 3, 22)), \
             patch.object(server, "peek_dashboard_snapshot", return_value=(*self._dashboard_snapshot(), "fresh")):
