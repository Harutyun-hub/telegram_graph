...
class SocialPostgresStoreQueryTests(unittest.TestCase):
    def _build_store(self) -> tuple[SocialPostgresStore, _CursorStub]:
        cursor = _CursorStub()
        store = _StoreUnderTest(cursor)
        return store, cursor

    def test_collect_claim_query_avoids_outer_join_for_update(self) -> None:
        store, cursor = self._build_store()

        store.claim_collect_accounts(
            worker_id="worker-1",
            platforms=["facebook"],
            limit=10,
            lease_seconds=60,
        ...
        self.assertEqual(params, (["facebook"], 60, 10, "worker-1"))

    def test_analysis_claim_query_avoids_outer_join_for_update(self) -> None:
        store, cursor = self._build_store()

        store.claim_analysis_activities(
            worker_id="worker-1",
            limit=20,
            lease_seconds=90,
        ...
        self.assertNotIn("LEFT JOIN public.social_processing_failures", query)
        self.assertIn("FOR UPDATE SKIP LOCKED", query)
        self.assertEqual(params, ("social-v1", 90, 20, "worker-1"))

    def test_graph_claim_query_avoids_outer_join_for_update(self) -> None:
        store, cursor = self._build_store()

        store.claim_graph_activities(
            worker_id="worker-1",
            limit=20,
            lease_seconds=120,
            projection_version="social-graph-v1",
        )
        ...
        self.assertNotIn("LEFT JOIN public.social_processing_failures", query)
        self.assertIn("FOR UPDATE SKIP LOCKED", query)
        self.assertEqual(params, ("social-graph-v1", 120, 20, "worker-1"))
