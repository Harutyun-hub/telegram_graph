...
async def require_operator_access(
    ...
    try:
        ...
        logger.warning(f"Operator auth validation failed: {exc}")
        raise HTTPException(
            status_code=401,
            detail="Your session could not be validated. Please sign in again.",
        ) from exc
...
async def require_kb_access(
    ...
    if token and any(hmac.compare_digest(token, valid) for valid in valid_tokens):
...
async def topic_detail(
    ...
    try:
        ctx = build_dashboard_date_context(from_date, to_date) if from_date and to_date else _default_dashboard_context()
        query_started_at = time.perf_counter()
        payload = get_topic_detail(topic, category, ctx)
        _record_query_timing(request, query_started_at)
        if payload is None:
...
make_kb_components = None


def _kb_components():
    """Lazy KB runtime bootstrap for routes that only need collection access in tests/admin flows."""
    global make_kb_components
    ...
    if make_kb_components is None:
        try:
            from api.knowledge_base import make_kb_components as factory
