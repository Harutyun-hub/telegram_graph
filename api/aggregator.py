...
def _with_cache_source(
    ...
    *,
    ...
    enriched = _with_refresh_state(cache_key, meta, stale_age_seconds=stale_age_seconds)
    enriched["cacheSource"] = cache_source
    return enriched
...
def _build_snapshot_sequential(
...
def _build_snapshot(
    ...
    *,
    ...
    if PARALLEL_ENABLED:
        data, tier_times = _build_snapshot_parallel(ctx, use_timeouts=use_timeouts, skipped_tiers=skipped_tiers)
    else:
        data, tier_times = _build_snapshot_sequential(ctx, skipped_tiers=skipped_tiers)
...
def _refresh_dashboard_snapshot(
    ...
    *,
    ...
    try:
        ...
        return data, _with_cache_source(cache_key, build_meta, cache_source="rebuild")
        ...
        if stale_snapshot is not None and stale_ts is not None:
            ...
            return stale_snapshot, _with_cache_source(
                cache_key,
                failure_meta,
                cache_source="memory",
                stale_age_seconds=max(0.0, time.time() - stale_ts),
...
def _ensure_background_refresh(cache_key: str, ctx: DashboardDateContext) -> dict[str, Any]:
    now = time.time()
    with _refresh_state_lock:
        state = _refresh_states.get(cache_key)
        if state is None:
...
def get_dashboard_snapshot(
    ...
    with _cache_lock:
        ...
        if entry is not None and not force_refresh and _is_cache_valid(cache_key, now):
            ...
            if not _should_bypass_cached_snapshot(snapshot, meta):
                return snapshot, _with_cache_source(cache_key, meta, cache_source="memory")
            logger.warning(
    ...
    if stale_entry is not None and STALE_WHILE_REVALIDATE and not force_refresh:
        refresh_status = _ensure_background_refresh(cache_key, resolved_ctx)
        stale_meta = dict(stale_entry[2])
        stale_meta["cacheStatus"] = (
            "stale_while_revalidate"
            if refresh_status["started"]
            else "stale_refresh_suppressed"
            if refresh_status["suppressed"]
            else "stale_while_revalidate_inflight"
        )
        stale_meta["isStale"] = True
        stale_meta["refreshSuppressed"] = bool(refresh_status["suppressed"])
        return stale_entry[1], _with_cache_source(
            cache_key,
            stale_meta,
            cache_source="memory",
        ...
        if state.event.wait(timeout=wait_timeout):
            ...
            if refreshed_entry is not None:
                ...
                if _is_cache_valid(cache_key, refreshed_now):
                    return refreshed_entry[1], _with_cache_source(
                        cache_key,
                        refreshed_entry[2],
                        cache_source="memory",
                    )
...
def peek_dashboard_snapshot(
    ...
    if _is_cache_valid(cache_key, now) and not _should_bypass_cached_snapshot(snapshot, meta):
        return snapshot, _with_cache_source(cache_key, meta, cache_source="memory"), "fresh"

    if _can_serve_stale(entry, now):
        stale_meta = dict(meta)
        stale_meta["isStale"] = True
        stale_meta.setdefault("cacheStatus", "memory_stale")
        return snapshot, _with_cache_source(
            cache_key,
            stale_meta,
            cache_source="memory",
    ...
    return None, _with_cache_source(cache_key, meta, cache_source="memory"), "expired"


def refresh_dashboard_snapshot_async(ctx: DashboardDateContext) -> bool:
    return bool(_ensure_background_refresh(ctx.cache_key, ctx).get("started"))
