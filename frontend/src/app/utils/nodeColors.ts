...
const colorPalette: Record<NodeType, NodeColors> = {
  brand: {
    core: '#06b6d4',      // Cyan-500 - Main brands (unchanged)
    glow: 'rgba(6, 182, 212, 0.6)',
    edge: '#22d3ee',      // Cyan-400
    darkEdge: '#0e7490',  // Cyan-700
  },
  topic: {
  ...
  competitor: {
    core: '#ec4899',      // Pink-500 - Competitor brands
  ...
  platform: {
    core: '#06b6d4',      // Cyan-500 - Social platforms (same as brand for consistency)
...
export function getNodeSize(type: NodeType | string, connections: number = 0): number {
  const baseSizes: Record<NodeType, number> = {
    brand: 18,        // Largest - primary entities
