...
export function useTopicDetail(topic: string | null, category: string | null) {
  ...
  const key = enabled ? `radar.details.topic.v5:${range.from}:${range.to}:${topic}:${category}` : null;
...
export function useTopicEvidenceFeed(
  ...
  const { range } = useDashboardDateRange();
  const active = Boolean(enabled && topic && category);
  ...
  const key = active ? `radar.feed.topic.v2:${range.from}:${range.to}:${topic}:${category}:${view}:${focusId || ''}` : null;
