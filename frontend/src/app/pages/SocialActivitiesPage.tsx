...
import {
  getSocialActivities,
  getSocialAdminEntities,
  getSocialOverview,
  getSocialRuntimeFailures,
  replaySocialActivities,
  retrySocialRuntimeFailure,
  runSocialRuntimeOnce,
  updateSocialEntity,
  type SocialActivity,
  type SocialEntity,
  type SocialOverviewResponse as SocialOverview,
  type SocialRuntimeFailure as SocialFailure,
} from '../services/socialIntelligence';
...
export function SocialActivitiesPage() {
  ...
  async function loadData(showSpinner: boolean) {
    ...
    try {
      const [overviewPayload, entitiesPayload, activitiesPayload, failuresPayload] = await Promise.all([
        getSocialOverview(),
        getSocialAdminEntities(),
        getSocialActivities({ limit: 80 }),
        getSocialRuntimeFailures({ deadLetterOnly: true, limit: 20 }),
      ]);
  ...
  async function handleSaveEntity(entity: SocialEntity) {
    ...
    setError(null);
    try {
      await updateSocialEntity(entity.id, {
  ...
  async function handleRetryFailure(failure: SocialFailure) {
    ...
    setError(null);
    try {
      await retrySocialRuntimeFailure(failure.stage, failure.scope_key);
      await loadData(false);
    } catch (err: any) {
  ...
  async function handleReplaySelected(stage: 'analysis' | 'graph') {
    ...
    setError(null);
    try {
      await replaySocialActivities(stage, [selectedActivity.activity_uid]);
      await loadData(false);
    } catch (err: any) {
